...
class Technicals():
    ...
    def fetch_candles(self, row_count):
        ...
        if df is None:
            self.log_message(f"error occured while fetching {self.pair}, time: {candle_time}, status code = {status_code}")
            return None
    ...
    def process_candles(self, df):
        ...
        df['MA_3'] =df.mid_c.rolling(window=3).mean()
        df['dy_mean']=(df.MA_3 - df.MA_3.shift(1))*100
